	...
	var ajaxService = app.service("AjaxService", [ "$log", "$http",
			function($log, $http) {
				...
				self.post = function(config) {
					...
					$log.log('post in ajaxService is called!');
					$log.log("config object in ajaxService Post Method: ", config);
				...
				self.get = function(config) {
					...
					$log.log('get  in AjaxService  called!');

					$log.log("config object in AjaxService get method: ", config);
